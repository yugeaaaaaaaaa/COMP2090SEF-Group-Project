...
class RecruitmentDatabase:
    ...
    def get_user_by_email(self, email: str):
        conn = self._get_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT user_id, name, email, password, user_type FROM users WHERE email = ?', (email,))
        row = cursor.fetchone()
        conn.close()
        if row:
            return {'user_id': row[0], 'name': row[1], 'email': row[2], 'password': row[3], 'user_type': row[4]}
        return None
