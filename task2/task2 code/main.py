...
def index():
    if 'user_id' in session:
        ...
        if session['user_type'] == 'recruiter':
            ...
            menu += '<a href="/my_skills">My Skills</a><br>'
...
def search():
    ...
    current_user = None
    if 'user_id' in session and session.get('user_type') == 'seeker':
        current_user = platform.current_user

    html = '<h1>Search Jobs</h1>'
    html += '<form method="get">'
    html += '<input name="keyword" placeholder="Enter keyword">'
    ...
    if jobs:
        for job in jobs:
            match_percent = 0
            if current_user:
                score = match_score(current_user, job, platform.jobs)
                match_percent = int(score * 100)

            if match_percent >= 80:
                badge_class = "high"
                badge_text = "recommondation"
            elif match_percent >=50:
                badge_class = "medium"
                badge_text = "you can try"
            elif match_percent > 0:
                badge_class = "low"
                badge_text = "no recommodation"
            else:
                badge_class = "no calclude"
                badge_text = "none"

            job_skills = ', '.join(job.skills) if job.skills else 'no skill'

            html += f'''
            <div>
                <h3>{job.title}</h3>
                <p>Company:{job.company}</p>
                <p>Description:{getattr(job, 'description', 'N/A')[:150]}...</p>
                <p class="skills"><strong>Required Skills:</strong> {job_skills}</p>
                <div class="match-badge {badge_class}">{badge_text} {match_percent}%</div>
...
def post_job():
    if request.method == 'POST':
        ...
        platform.post_job(title,description,company, skills)
        return redirect(url_for('index'))
        ...
        for job in my_jobs:
            ...
            if applicants:
                ...
                for app_info in applicants:
                    ...
                    match_percent = app_info.get('match_percent', 0)
                    ...
                    if match_percent >= 80:
                        ...
                        match_class = "no caclude"

                    html+= f'''
                    <p><strong>Name:</strong> {applicant.name}</p>
                    <p><strong>Email:</strong> {applicant.email}</p>
                    <div style="margin: 10px 0;">
                            <span class="{match_class}"><strong>match percent:</strong>{match_percent}%</span>
                    </div>
                    <p><strong>Status:</strong> 
                    <span style="padding:2px 6px; border-radius:3px; background:#e0e0e0;">{application.status}</span></p>
...
def my_skills():
    ...
    if session.get('user_type') != 'seeker':
        return "Only job seekers can add skills."
    
    if platform.current_user is None:
        return "User not found. Please login again. <a href='/login'>Login</a>"
    
    if request.method == 'POST':
        skills_text = request.form.get('skills', '')
        skills_list = [s.strip() for s in skills_text.split(',') if s.strip()]
        platform.update_user_skills(skills_list)
        return redirect(url_for('my_skills'))
    
    current_skills = ', '.join(platform.current_user.skills) if platform.current_user.skills else ''
    
    return f'''
    <h1>My Skills</h1>
    <form method="post">
        <label>Your Skills (comma-separated):</label><br>
        <input name="skills" size="50" value="{current_skills}"><br><br>
